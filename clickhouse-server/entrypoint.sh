...
cp /macros/"$local_ip"_macros.xml /etc/clickhouse-server/config.d/

#删除IPv6配置
sed -i 's/<listen_host>::<\/listen_host>//g' /etc/clickhouse-server/config.d/docker_related_config.xml

#解决docker部署网络不通的问题
sed -i "s/example.clickhouse.com/$local_ip/g" /etc/clickhouse-server/config.xml

#解决docker cap ipc lock的问题
